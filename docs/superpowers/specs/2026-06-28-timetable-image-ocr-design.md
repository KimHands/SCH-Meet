...
```json
{
  ...
  "warnings": ["요일 헤더를 일부 인식하지 못해 균등 분할로 추정했습니다"]
}
```
...
3. **셀 클러스터링 → 과목**
   ...
   - 클러스터: `name`=단어 join, `day`=열, `start/end_minute`=클러스터 위/아래 y → 회귀 환산
     후 **5분 단위 반올림**
   - 08:00~22:00 클램프, end≤start거나 <20분 클러스터 폐기
